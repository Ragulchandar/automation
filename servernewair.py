...
if lineplot:
    if uploaded_file_2 is not None and uploaded_file_1 is not None:
        ...
        df_plt= df_1[[Gases,"Time"]]
        
        #corr_df=cor(clean_df)
        fig, ax = plt.subplots()
        fig = st.line_chart(df_plt, x="Time", height=500)
...
Ftir = st.selectbox('Select FTIR variables', options=['CO2 35C', 'Acetaldehyde  35c', 'Formaldehyde 35c','Toluene 35c', 'o3 35c hitran', 'ETHYLENE 35C', 'ACETONE 35C', 'Octane 35c', 'Butane 25c', 'H2O% 35C', 'Temp (C)', 'Pressure (Atm)', 'Ethanol 35C', 'IPA 35C'], key='1')
Ftir1 = st.selectbox('Select FTIR variables', options=['CO2 35C', 'Acetaldehyde  35c', 'Formaldehyde 35c','Toluene 35c', 'o3 35c hitran', 'ETHYLENE 35C', 'ACETONE 35C', 'Octane 35c', 'Butane 25c', 'H2O% 35C', 'Temp (C)', 'Pressure (Atm)', 'Ethanol 35C', 'IPA 35C'], key='11')
Ftir2 = st.selectbox('Select FTIR variables', options=['CO2 35C', 'Acetaldehyde  35c', 'Formaldehyde 35c','Toluene 35c', 'o3 35c hitran', 'ETHYLENE 35C', 'ACETONE 35C', 'Octane 35c', 'Butane 25c', 'H2O% 35C', 'Temp (C)', 'Pressure (Atm)', 'Ethanol 35C', 'IPA 35C'], key='12')
Ftir3 = st.selectbox('Select FTIR variables', options=['CO2 35C', 'Acetaldehyde  35c', 'Formaldehyde 35c','Toluene 35c', 'o3 35c hitran', 'ETHYLENE 35C', 'ACETONE 35C', 'Octane 35c', 'Butane 25c', 'H2O% 35C', 'Temp (C)', 'Pressure (Atm)', 'Ethanol 35C', 'IPA 35C'], key='14')
Cubic= st.selectbox('Select Cubic variables', options=['GRIMM PM1.0', 'GRIMM PM2.5', 'GRIMM PM10', 'TSI PM1.0', 'TSI PM2.5', 'TSI PM10', 'VOC', 'CO2', 'Temperature', 'humidity'], key='4')
Airthings= st.selectbox('Select airthings variables', options=['TEMP', 'HUMIDITY %', 'PRESSURE hPa', 'CO2 ppm_x', 'VOC ppb', 'PM1 g/m3', 'PM2_5 g/m3'], key='7')
Tsi= st.selectbox('Select TSI variables', options=['CO2 ppm_y', 'VOC tVOC measurement mg/m3', 'Temperature Fahrenheit', 'Relative Humidity %', 'PM2.5 ug/m3', 'PM10 ug/m3'],key='8')
sen1= st.selectbox('Select SEN value', options=['SEN55 value'],key=41)
sen2= st.selectbox('Select SEN value', options=['SEN55 value 2'],key=42)
...
if lineplot:
    if uploaded_file_2 is not None and uploaded_file_1 is not None:
        ...
        df_plt= out_df[[Ftir, Cubic, Airthings, Tsi]]
        #set y axis range as 15 to 20:
        
        #corr_df=cor(clean_df)
        # fig, ax = plt.subplots()
        fig = st.line_chart(df_plt, height=0)
